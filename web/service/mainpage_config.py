...
class MainPageRemoteConfig:
    ...
    async def refresh_banner(self) -> None:
        ...
        for r in bitable.table(BANNER_KEY).view(VIEW_NAME).rows():
            try:
                ...
                image_filepath = await self.download_resource(image.url)

                logger.info(
                    f"downloading banner video, media type {video.media_type}, url {video.url} ..."
                )
                video_filepath = await self.download_resource(video.url)
        ...
        banner = mainpage.Banner(banners=banner_items)
        await banner.save()

    async def refresh_magic(self) -> None:
        logger.info("pulling magic config from remote ...")
        bitable = Bitable(self._auth_token, self._bid)

        magic_name_by_id: dict[str, str] = {}
        showcases_by_name: dict[str, list[mainpage.ShowcaseItem]] = {}
        prompts_by_name: dict[str, list[mainpage.PromptItem]] = {}
        ...
        for r in bitable.table("magic showcase").view("表格").rows():
            try:
                ...
                original_filename = await self.download_resource(r.col("原始图").url)
                result_filename = await self.download_resource(r.col("处理后图").url)
            except KeyError:
                continue

            sc = mainpage.ShowcaseItem(
        ...
        for r in bitable.table("magic prompt").view("表格").rows():
            ...
            p = mainpage.PromptItem(name=name, prompt=prompt)
    ...
    async def refresh_shortcut(self) -> None:
        ...
        for r in (
            ...
            magic_file_name = await self.download_resource(magic.url)
            teach_file_name = await self.download_resource(teach.url)
            params_body = json.loads(params.text)

            s = mainpage.ShortcutItem(
                type=_type.value,
        ...
        shortcut = mainpage.Shortcut(shortcuts=shortcuts)
        await shortcut.save()
    ...
    async def download_resource(self, url: str) -> str:
        ...
        async with oss.save_file(object_name, media_type) as writer:
            await writer.write_bytes(resp.content)
            logger.info(
                f"resource download complete, type: {media_type}, size: {size}, filename {object_name}"
            )
            return writer.file_id
