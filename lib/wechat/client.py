...
class CryptoHelper:
    ...
    @staticmethod
    def make_auth_str(sec: Secrets, timestamp: str, nonce: str, sign: str) -> str:
    ...
    @staticmethod
    def signature(sec: Secrets, method: str, url: str, body: str) -> dict[str, str]:
    ...
    @staticmethod
    def verify(sec: Secrets, timestamp: str, nonce: str, sign: str, data: str) -> bool:
        data = f"{timestamp}\n{nonce}\n{data}\n"
        return crypto.sha256_with_rsa_verify(sec.pub_key, sign.encode(), data)


def new_client(secerts: Secrets) -> "WxClient":
